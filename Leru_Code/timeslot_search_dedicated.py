...
if __name__ == "__main__":
    ...
    # -----------------------------
    # Grid search (Dedicated)
    # -----------------------------
    rows = []

    for ts1 in type1_range:
        for ts2 in type2_range:
            # collect replication KPI values for this combo
            rep_d_overtime_total = []
            rep_d_overtime_m0 = []
            rep_d_overtime_m1 = []
            ...
            for rep in range(num_replications):
                # deep copy so we don't overwrite the stored patient list
                ...
                rep_d_overtime_m0.append(m0)
                rep_d_overtime_m1.append(m1)
                rep_d_overtime_total.append(tot)
            ...
            # aggregate (mean + std) for this combo
            row = {
                "timeslot_type1": ts1,
                "timeslot_type2": ts2,

                "mean_overtime_total_min": float(np.mean(rep_d_overtime_total)),
                "std_overtime_total_min": float(np.std(rep_d_overtime_total, ddof=1)),
    ...
    # -----------------------------
    # Identify "best" (dedicated)
    # -----------------------------
    # Default ranking: minimize total overtime, then minimize avg waits, then minimize extra waits
    ...
    # Save full table + top 10
    results_sorted.to_csv("dedicated_timeslot_grid_results3.csv", index=False)
    results_sorted.head(10).to_csv("dedicated_timeslot_grid_top103.csv", index=False)

    # Print best combo
    best = results_sorted.iloc[0]
    print("\nBEST (Dedicated) by: overtime_total -> avg_waits -> extra_waits")
    ...
    print("\nSaved: dedicated_timeslot_grid_results.csv and dedicated_timeslot_grid_top10.csv")
